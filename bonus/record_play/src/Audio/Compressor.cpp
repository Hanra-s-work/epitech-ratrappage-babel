...
std::optional<std::any> Audio::Compressor::compressAudio(const StreamData &node)
{
    PRETTY_WARNING << "Audio compression is not implemented yet." << std::endl;
    return std::nullopt;
}

const Audio::StreamData Audio::Compressor::decompressAudio(const std::any &stream)
{
    PRETTY_WARNING << "Audio decompression is not implemented yet." << std::endl;
    StreamData node;
    node.duration = 0;
    node.samples = std::vector<float>();
    return node;
}
